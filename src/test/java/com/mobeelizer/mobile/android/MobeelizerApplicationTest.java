...
@PrepareForTest({ MobeelizerApplication.class, Bundle.class,
		AssetManager.class, MobeelizerDefinitionParser.class, Log.class,
...
public class MobeelizerApplicationTest {
	...
	public void init() throws Exception {
		...
		PowerMockito.when(Log.class, "i", anyString(), anyString()).thenReturn(
				0);

		PowerMockito.mockStatic(Environment.class);
		PowerMockito.when(Environment.class, "getExternalStorageState")
		...
		when(mobeelizer.getSystemService(Context.TELEPHONY_SERVICE))
		...
		when(bundle.getString("MOBEELIZER_DEFINITION_ASSET")).thenReturn(
				"definition.xml");
		when(bundle.getString("MOBEELIZER_DB_NAME")).thenReturn("databaseName");
		when(bundle.getString("MOBEELIZER_PACKAGE")).thenReturn(
				"com.mobeelizer.orm");
		...
		when(
				packageManager.getApplicationInfo("packageName",
						PackageManager.GET_META_DATA)).thenReturn(
				applicationInfo);
		...
		when(assetManager.open("definition.xml"))
				.thenReturn(definitionXmlAsset);
		...
		PowerMockito.whenNew(MobeelizerDefinitionParser.class)
				.withNoArguments().thenReturn(xmlUtil);

		PowerMockito.mockStatic(MobeelizerDefinitionParser.class);
		PowerMockito.when(MobeelizerDefinitionParser.class, "parse",
				definitionXmlAsset).thenReturn(definition);

		connectionManager = mock(MobeelizerRealConnectionManager.class);

		PowerMockito.whenNew(MobeelizerRealConnectionManager.class)
				.withArguments(any(MobeelizerApplication.class))
				.thenReturn(connectionManager);

		definitionManager = mock(MobeelizerDefinitionConverter.class);

		PowerMockito.whenNew(MobeelizerDefinitionConverter.class)
				.withNoArguments().thenReturn(definitionManager);

		models = new HashSet<MobeelizerModel>();

		when(
				definitionManager.convert(definition, "com.mobeelizer.orm",
						"role")).thenReturn(models);

		internalDatabase = mock(MobeelizerInternalDatabase.class);

		PowerMockito.whenNew(MobeelizerInternalDatabase.class)
				.withArguments(any(MobeelizerApplication.class))
		...
		MobeelizerLoginResponse loginResponse = new MobeelizerLoginResponse(
				MobeelizerLoginStatus.OK, "0000", "role", false);

		when(connectionManager.login()).thenReturn(loginResponse);
		when(connectionManager.isNetworkAvailable()).thenReturn(true);

		PowerMockito.whenNew(MobeelizerDatabaseImpl.class)
				.withArguments(eq(application), eq(models))
				.thenReturn(database);

		syncPerformer = PowerMockito.mock(MobeelizerSyncServicePerformer.class);
		PowerMockito.whenNew(MobeelizerSyncServicePerformer.class)
				.withArguments(any(MobeelizerApplication.class), anyBoolean())
	...
	public void shouldCreateWithDefaultDefinitionXml() throws Exception {
		...
		when(assetManager.open("application.xml")).thenReturn(
				definitionXmlAsset);

		// when
		MobeelizerApplication application = MobeelizerApplication
				.createApplication(mobeelizer);
	...
	public void shouldCreateFailIfDefinitionXmlAssetNotFound() throws Exception {
		// given
		when(bundle.getString("MOBEELIZER_DEFINITION_ASSET")).thenReturn(null);
		when(assetManager.open("application.xml")).thenThrow(
	...
	public void shouldCreateInDevelopmentMode() throws Exception {
		// given
		when(bundle.getString("MOBEELIZER_DEVELOPMENT_ROLE")).thenReturn(
				"devRole");
		when(bundle.getString("MOBEELIZER_MODE")).thenReturn("development");

		MobeelizerDevelopmentConnectionManager connectionManager = new MobeelizerDevelopmentConnectionManager(
				"devRole");
		PowerMockito.whenNew(MobeelizerDevelopmentConnectionManager.class)
				.withArguments("devRole").thenReturn(connectionManager);

		when(
				definitionManager.convert(definition, "com.mobeelizer.orm",
						"devRole")).thenReturn(models);
		...
		PowerMockito.whenNew(MobeelizerDatabaseImpl.class)
				.withArguments(eq(application), eq(models))
				.thenReturn(database);
		verifyNew(MobeelizerDevelopmentConnectionManager.class).withArguments(
				"devRole");
		assertEquals(MobeelizerLoginStatus.OK,
				application.login("anyInstance", "anyUser", "anyPassword"));
	...
	public void shouldCreate() throws Exception {
		// then
		verifyNew(MobeelizerRealConnectionManager.class).withArguments(
				application);
	...
	public void shouldLoginFail() throws Exception {
		// given
		MobeelizerLoginResponse loginResponse = new MobeelizerLoginResponse(
				MobeelizerLoginStatus.AUTHENTICATION_FAILURE);
		when(connectionManager.login()).thenReturn(loginResponse);

		// when
		MobeelizerLoginStatus status = application.login("instance", "user",
				"other-password");

		// then
		assertEquals(MobeelizerLoginStatus.AUTHENTICATION_FAILURE, status);
		verifyNew(MobeelizerDatabaseImpl.class, never()).withArguments(
				eq(application), anySet());
	}
	...
	public void shouldLoginWithInitialSync() throws Exception {
		// when
		MobeelizerLoginResponse loginResponse = new MobeelizerLoginResponse(
				MobeelizerLoginStatus.OK, "0000", "role", true);
		when(connectionManager.login()).thenReturn(loginResponse);
		when(connectionManager.isNetworkAvailable()).thenReturn(true);
	...
	public void shouldChangeSyncStatus() throws Exception {
		...
		// then
		assertEquals(MobeelizerSyncStatus.FILE_RECEIVED,
				application.checkSyncStatus());
	}
